...
import { ASC_MIN_GOLD, ASC_NODES, STONE_RESONANCE, ascCost, startGoldFor, stonesFor } from '../../data/ascension';
...
export function ascendView(s: GameState, m: Mods): TemplateResult {
  const gain = stonesFor(s.stats.runGold, m.stoneGain);
  return html`<div class="view">
    ...
    <div class="card highlight">
      <div class="row between">
        ...
        <span class="muted">Ascensions: ${s.asc.count}</span>
      </div>
      ...
      ${s.stats.runGold < ASC_MIN_GOLD
        ? html`${bar(s.stats.runGold / ASC_MIN_GOLD, '#f5c542', `${fmt(s.stats.runGold)} / ${fmt(ASC_MIN_GOLD)} gold earned this run`)}
            <div class="dim">Earn ${fmt(ASC_MIN_GOLD)} gold in a single run to ascend. Stones scale with the square root of gold earned.</div>`
