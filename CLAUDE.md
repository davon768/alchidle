...
- **Balance bot**: `npm run bot` (`scripts/balance-bot.ts`, run under plain Node via the extensionless-import hook in `scripts/register-ts.mjs`). `npm run bot -- 12 --runs 5 --json`; `--no-research` and `--single-herb` switch a system off so its contribution can be isolated. It plays perfectly; humans are ~2× slower. **Its absolute times are sensitive to the bot's own policy** — changing only the order in which it hires apprentices moved first ascension from 65 to 97 minutes — so treat a single number as unvalidated and compare configurations with the policy held fixed instead. Current targets: first ascension ≈ 70 bot-minutes (~2–3 h human); a Common apprentice reaches its cap ≈ 1.5 h after hire; the most-used item reaches proficiency 50 in ≈ 3 h while 100 takes dozens of hours; combat harness: arriving with previous-tier gear clears ~40–85% of a dungeon, farmed current-tier gear nearly clears it.
